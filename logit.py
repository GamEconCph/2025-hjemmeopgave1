...
def choice_prob(theta, x):
    ...
    assert x.ndim == 3, f'x should have ndim == 3, got {x.ndim}: unable to infer which dimension is whichs'
    N, J, K = x.shape
...
def elasticity_numeric(theta, x, k=0, h=1e-2): 
    ...
    print(f'Own-price elasticity: {E_own.mean().round(4)}')

    print(f'Cross-price elasticity: {E_cross.mean().round(4)}')
